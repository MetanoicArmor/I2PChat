...
- Security / аудит: строгая проверка конца передачи файла (`E`), предупреждения по trust / legacy / BlindBox, корректная привязка `legacy_compat` к codec, зафиксированный SHA256 архива `gitleaks` в CI, расширенный test gate, обновлённые `docs/AUDIT_EN.md` / `docs/AUDIT_RU.md`.
