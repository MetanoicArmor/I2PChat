...
- **Safer pinned-key recovery in GUI:** the `Forget pinned peer key` action lets users remove a stale TOFU pin from the UI instead of editing `trust.json` manually, and now requires explicit confirmation before the pin is forgotten.
...
- **Более безопасное восстановление pinned key в GUI:** действие `Forget pinned peer key` позволяет удалить устаревший TOFU pin прямо из UI без ручного редактирования `trust.json`, и теперь требует явного подтверждения перед удалением.
