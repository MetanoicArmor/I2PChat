...
- **Group invites:** any current member can **Copy invite** (group context menu) to get a shareable `__I2PCHAT_GROUP_INVITE__:` string (same out-of-band trust model as copying your address). The recipient uses **⋯ → Join group via invite…** (or TUI `/group join …` / F8 **Join invite**) to redeem it. Joining creates the local group from the invite snapshot and announces `op=join` to existing members. Invites are multi-use; there is no revoke/TTL in this release.
