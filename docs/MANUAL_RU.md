...
- **Инвайты в группы:** любой текущий участник может **Copy invite** (контекстное меню группы) и получить строку `__I2PCHAT_GROUP_INVITE__:` (тот же out-of-band trust, что и при копировании своего адреса). Получатель вставляет её через **⋯ → Join group via invite…** (или TUI `/group join …` / F8 **Join invite**). Вход создаёт локальную группу из снимка инвайта и рассылает `op=join` текущим участникам. Инвайты многоразовые; revoke/TTL в этом релизе нет.
