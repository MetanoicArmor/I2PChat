...
- **Область чата** — в верхней части окна отображаются сообщения (ваши и собеседника), системные уведомления, статус передачи файлов. Сообщения можно выделять и копировать (правый клик или контекстное меню).
...
- **Строка статуса** — под полем ввода отображается состояние подключения (например, «Connected» или «Disconnected»).
...
- **Copy my address** — скопировать ваш I2P-адрес в буфер (то же, что раньше «Copy My Addr»).
...
#### 4.5. Кнопка `Copy My Addr`

Кнопка **`Copy My Addr`** копирует ваш собственный I2P‑адрес в буфер обмена.

<img src="../screenshots/4.png" alt="Получение собственного адреса через Copy My Addr" width="900" />
