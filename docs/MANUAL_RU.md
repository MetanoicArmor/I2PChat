...
#### 4.9. Опционально: BlindBox (офлайн-доставка)

**BlindBox** — функция офлайн-доставки для закреплённого пира, когда **нет живой защищённой сессии**. Для **именованных** профилей (`persistent`) включена по умолчанию; для эфемерного профиля `random_address` отключена.

- Нужен **постоянный профиль** и **Lock to peer**. Для межхостовой офлайн-доставки задайте общие **Blind Box**-серверы через `I2PCHAT_BLINDBOX_REPLICAS`. Для дефолта на весь деплой используйте `I2PCHAT_BLINDBOX_DEFAULT_REPLICAS`. Для централизованной прод-настройки — `I2PCHAT_BLINDBOX_DEFAULT_REPLICAS_FILE`. В **готовых сборках** дополнительно зашит **один публичный endpoint** в `DEFAULT_RELEASE_BLINDBOX_ENDPOINTS` в `i2pchat/core/i2p_chat_core.py` (`dzyhukukogujr6r2vwfy667cwm7vg3oomhx2sryxhb6mn4i4wbjq.b32.i2p:19444`; перекрывается переменными выше; отключить: `I2PCHAT_BLINDBOX_NO_BUILTIN_DEFAULTS=1`). См. [**RELEASE_0.6.0.md**](releases/RELEASE_0.6.0.md) — без повторения криптодеталей).
  ...
  **Локальный токен:** задайте **`I2PCHAT_BLINDBOX_LOCAL_TOKEN`** в окружении процесса **I2PChat** (и тем же значением — отдельный демон-реплику на том же `host:port`, если он у вас есть). В режиме **`local-auto`**, если переменная не задана, ядро генерирует одноразовый токен на запуск (удобно для простой разработки, но не для стыковки с внешним процессом).
  **Секрет на реплику (именованные профили):** в окне **BlindBox diagnostics** можно задать опциональные токены для конкретных endpoint’ов (по одной строке: `endpoint<TAB>токен`). Они хранятся в `<profile>.blindbox_replicas.json` в поле **`replica_auth`** (формат файла **версия 2**; старые файлы **версии 1** читаются как только список реплик). Клиент подставляет токен в probe и queue-команды (`CAPA` / `QPUT` / `QGET` / `QDEL`) только для этого адреса. На своей Python-реплике задайте **`BLINDBOX_AUTH_TOKEN`** тем же значением (см. `i2pchat/blindbox/blindbox_server_example.py`). Токен в протоколе **не заменяет** доверие к I2P destination — он лишь ограничивает доступ к сырому TCP-протоколу.
...
- `Send` в GUI работает как «умный маршрут»:
  - при живой защищённой сессии отправляет текст и файлы онлайн;
  - при `Send: offline queue` ставит текст, файлы и inline-картинки в BlindBox-очередь (без обязательного ручного Connect);
...
- **Совместимость:** BlindBox-реплики обязаны поддерживать новый queue-capability протокол. Endpoint’ы без него отклоняются, а не используются через ослабленный fallback.
- **Лимиты размера:** live-передача файлов допускает до **2000 MB**; BlindBox/offline-передача файлов ограничена **200 MB**.

Как новый BlindBox работает на практике:

```mermaid
flowchart LR
    A["Alice"] -->|"QPUT в mailbox A→B
текст / файл / картинка"| R["BlindBox replicas"]
    R -->|"QGET из mailbox A→B"| B["Bob"]
    B -->|"QPUT отложенного receipt
MSG_ACK / FILE_ACK / IMG_ACK"| R
    R -->|"QGET receipt из mailbox B→A"| A
```

Пример:

1. Алиса хочет отправить Бобу сообщение, файл или inline-картинку, пока Боб оффлайн.
2. Клиент Алисы вычисляет **однонаправленный mailbox** для направления **Alice → Bob**.
3. У этого mailbox есть отдельные capability-права на:
   - запись blob'ов в очередь;
   - чтение blob'ов из очереди;
   - удаление blob'ов после получения.
4. Алиса кладёт зашифрованные оффлайн-blob'ы в BlindBox через `QPUT`.
5. Когда Боб возвращается, его клиент читает mailbox через `QGET`, собирает сообщение/файл/картинку и удаляет уже обработанные blob'ы.
6. Затем клиент Боба может отправить отложенный receipt назад через обратный mailbox **Bob → Alice**.
7. Когда Алиса позже опрашивает BlindBox, она получает `MSG_ACK`, `FILE_ACK` или `IMG_ACK`, даже если в момент чтения у Боба не было живой сессии с ней.

Зачем нужна mailbox rotation:

- Mailbox не должен оставаться одним и тем же навсегда.
- Приложение может перевести направление **Alice → Bob** на новый epoch mailbox.
- Новый оффлайн-трафик идёт уже в новый mailbox, а предыдущий ещё живёт grace-период, чтобы не потерять задержавшиеся blob'ы.
- Это уменьшает долгоживущую корреляцию по сравнению с одной постоянной оффлайн-очередью на весь срок жизни контакта.
...
Пункт меню **⋯ → Check for updates…** (**Ctrl/Cmd+U**) загружает **HTML** страницы релизов (для адресов `*.i2p` обычно через HTTP-прокси I2P, см. `I2PCHAT_UPDATE_HTTP_PROXY`), находит имена ZIP по шаблону и **сравнивает номер версии** с локальной (файл **`VERSION`** в корне при запуске из исходников). Приложение **не скачивает** архив и **не проверяет** хеш или подпись.
