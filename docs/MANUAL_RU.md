...
- **Поле ввода** — под областью чата: введите текст. **Enter** делает новую строку, а отправка сообщения — **Shift+Enter** (а также **Ctrl+Enter**, и на macOS **⌘+Enter**). Или используйте кнопку отправки. **Растровое изображение** из буфера можно вставить (**Ctrl+V** / **⌘V** или **Paste** в меню поля) — отправка как у пункта **Send picture** (PNG, JPEG, WebP).
