...
- **Поле ввода** — под областью чата: введите текст. **По умолчанию** на **macOS** **Enter** отправляет, а **Shift+Enter** вставляет новую строку; на **Windows/Linux** **Enter** вставляет новую строку, а **Ctrl+Enter** отправляет. На **macOS** в обоих режимах отправка срабатывает и по **⌘+Enter**, и по **Ctrl+Enter**. В меню **`⋯`** можно переключить **Enter sends message: ON/OFF** под свой привычный режим: при **ON** **Enter** отправляет, при **OFF** **Enter** вставляет новую строку. Подсказка-плейсхолдер в поле соответствует режиму. Настройка сохраняется в **`ui_prefs.json`**. **Растровое изображение** из буфера можно вставить (**Ctrl+V** / **⌘V** или **Paste** в меню поля) — отправка как у пункта **Send picture** (PNG, JPEG, WebP). Текст в поле ввода хранится **отдельно для каждого пира** (по нормализованному адресу): при смене адреса в панели действий или при смене текущего пира восстанавливается черновик для выбранного контакта; после успешной отправки черновик для этого пира очищается. Черновики сохраняются на диск в файл `profiles/<профиль>/<профиль>.compose_drafts.json` (шифруются ключом личности; с ограничением числа записей; при старой плоской раскладке читается и переносится в эту папку).
...
Слева от чата — список **Saved peers**: локальная **книга контактов** текущего профиля. Данные хранятся в `profiles/<профиль>/<профиль>.contacts.json` (рядом с `<профиль>.dat`) и на диске шифруются ключом личности профиля; старый plaintext JSON мигрирует при следующем сохранении.

- **Строки** — отображаемое имя (или сокращённый base32‑идентификатор без суффикса), подпись (превью последнего сообщения или ваша заметка), подсветка непрочитанного, если это не активный чат.
...
- **Enter sends message: ON/OFF** — при **ON**: **Enter** отправляет сообщение, **Shift+Enter** — новая строка (**Ctrl/⌘+Enter** по-прежнему отправляет). При **OFF**: **Enter** вставляет новую строку, **Shift+Enter** тоже вставляет новую строку, а **Ctrl+Enter** отправляет (**на macOS: ⌘+Enter или Ctrl+Enter**). Текст-подсказка в поле ввода меняется вместе с режимом; выбор сохраняется (см. **`ui_prefs.json`**).
...
Поле адреса пира принимает **канонический идентификатор**: строка **base32** (обычно 40 символов) **без** суффикса `.b32.i2p`. Такой же вид используется внутри приложения (контакты, сессии, группы).

При вводе из буфера по-прежнему принимается и полная форма **`<base32>.b32.i2p`** — она нормализуется к базовому host.

**Copy my address** копирует в буфер **канонический** адрес — **base32 без** суффикса `.b32.i2p` (как в контактах и поле пира).
...
**Несколько live-сессий:** можно держать несколько параллельных SAM-потоков к разным сохранённым пирам (лимит по умолчанию **8**, задаётся переменной **`I2PCHAT_MAX_LIVE_SESSIONS`**, диапазон 1–64). **`Connect`** остаётся доступной, если уже есть живой чат, но выбранный пир ещё не подключён и лимит не исчерпан. Повторное подключение к тому же пиру отклоняется.
...
**Состояние кнопки:** **`Connect`** **неактивна** (серая), пока в статусе сети не появятся **Pending** или **Visible** (сессия I2P готова), пока нет адреса пира или сохранённого закреплённого пира, пока уже есть **live-сессия с этим же пиром**, пока достигнут лимит одновременных сессий, либо пока идёт исходящая попытка подключения. Пока выполняется dial-out, **`Connect`** остаётся неактивной; повторное нажатие ядро игнорирует. **Подсказки** на кнопке объясняют причину (дождаться Pending/Visible, ввести адрес, уже подключены к этому пиру, лимит сессий и т.д.) и содержат строку **Shortcut:** для **Ctrl+1** / **⌘1**.  
Если BlindBox уже готов к офлайн-очереди, tooltip у `Connect` явно помечает подключение как **optional**.

#### 4.4. Кнопка `Disconnect`

Кнопка **`Disconnect`** разрывает **активную** live-сессию (пир, выбранный для чата; при одном потоке поведение как раньше). При нескольких параллельных сессиях отключается поток активного пира.
...
2. Если адрес уже есть:
   - в буфер обмена копируется строка **base32** (без `.b32.i2p`);
...
- После завершения **живой защищённой сессии** (handshake) подпись снова **`Send`** — **сразу**, без необходимости отправлять первое сообщение, чтобы кнопка обновилась.
...
- **Текстовые группы:** офлайн-доставка в группу **не использует отдельный «групповой» ключ BlindBox**. Сообщение рассылается **каждому** участнику (кроме отправителя) по **тому же парному** каналу BlindBox, что и личный чат с этим пиром: нужен **BlindBox root** с каждым таким участником (обычно после хотя бы одной успешной защищённой live-сессии 1:1). К подключённым в данный момент пирам сообщение может уйти по live, к остальным — в очередь BlindBox, если для пары ключи готовы. После отправки в ленте группы показывается сводка доставки; при сбоях по отдельным пирам — строка **Details** с причиной (например `blindbox-await-root`).
- **Инвайты в группы:** любой текущий участник может **Copy invite** (контекстное меню группы) и получить непрозрачный токен (тот же out-of-band trust, что и при копировании своего адреса). В строке не видно названия группы и адресов участников. Получатель вставляет её через **⋯ → Join group via invite…** (или TUI `/group join …` / F8 **Join invite**). Вход создаёт локальную группу из снимка инвайта и рассылает `op=join` текущим участникам. Инвайты многоразовые; revoke/TTL в этом релизе нет.
...
3. Проверьте отсоединённую подпись GPG для `SHA256SUMS` (**`SHA256SUMS.asc`**) ключом релизов (**отпечаток** `2BA0C56D8240077F9773248A2C05CFB3F6DFDF99`, UID **metanoicarmor@gmail.com** — на [keys.openpgp.org](https://keys.openpgp.org/search?q=metanoicarmor%40gmail.com)). При необходимости: `gpg --keyserver keys.openpgp.org --recv-keys 2BA0C56D8240077F9773248A2C05CFB3F6DFDF99`, затем `gpg --verify SHA256SUMS.asc SHA256SUMS`.
...
6. После установления соединения:
   ...
   - отправьте сообщение клавишей `Enter` на macOS или `Ctrl+Enter` на Windows/Linux; при желании можно нажать кнопку `Send`.
...
#### 7.6. Сборка из исходников: **uv** и слой **i2pchat.sam** (для разработчиков)

Если вы запускаете I2PChat из **git-репозитория** (не из готового архива):

- установите **[uv](https://docs.astral.sh/uv/)** и синхронизируйте зависимости по **`pyproject.toml`** / **`uv.lock`** — см. **README** репозитория (`uv sync`, затем `uv run python -m …`);
- **I2P SAM** (управляющее соединение с роутером, сессии, потоки, NAMING) реализован **в этом репозитории** пакетом **`i2pchat.sam`**. Пакет **`i2plib`** с PyPI для I2PChat **не используется**, вендорная копия удалена.
...
**Диагностика:** переменная **`I2PCHAT_LOG_LEVEL`** (`DEBUG` или `INFO`) включает вывод логов пакета `i2pchat` в stderr (фрейминг, HMAC, транспорт). Пример: `I2PCHAT_LOG_LEVEL=DEBUG python3 -m i2pchat.gui`.
