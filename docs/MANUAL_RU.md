...
Документ актуален для **I2PChat 1.0.0** (номер версии в корне репозитория: [`VERSION`](../VERSION)).
...
- **`default`**:
  ...
  - важный момент безопасности: в режиме `default` TOFU‑пины доверия не сохраняются между перезапусками;
- **выбор из списка**:
...
- **Поле ввода** — под областью чата: введите текст. **Enter** делает новую строку, а отправка сообщения — **Shift+Enter** (а также **Ctrl+Enter**, и на macOS **⌘+Enter**). Или используйте кнопку отправки. **Растровое изображение** из буфера можно вставить (**Ctrl+V** / **⌘V** или **Paste** в меню поля) — отправка как у пункта **Send picture** (PNG, JPEG, WebP). Текст в поле ввода хранится **отдельно для каждого пира** (по нормализованному адресу): при смене адреса в панели действий или при смене текущего пира восстанавливается черновик для выбранного контакта; после успешной отправки черновик для этого пира очищается. Черновики сохраняются на диск в файл `{profiles}/<профиль>.compose_drafts.json` (с ограничением числа записей).
...
Слева от чата — список **Saved peers**: локальная **книга контактов** текущего профиля. Данные хранятся в `<профиль>.contacts.json` в каталоге профилей (рядом с `.dat`).

<img src="../screenshots/6.png" alt="Главное окно с открытой панелью Saved peers: список контактов и чат" width="900" />
...
- **Privacy mode: ON/OFF** — быстрый пресет приватности (скрытие превью в уведомлениях и связанное «тихое» поведение); подпись показывает состояние.
- **Notification sound: ON/OFF** — звук входящих; при OFF путь к кастомному звуку сохраняется.
- **Hide message in notifications: ON/OFF** — при ON в трее не показывается текст сообщения (заголовок может содержать имя/адрес пира).
- **Quiet mode (focused): ON/OFF** — пока окно в фокусе, не показывать тосты и не проигрывать звук (в т.ч. для других чатов).
...
**Состояние кнопки:** **`Disconnect`** **неактивна**, пока нет активной сессии с пиром; при наведении показывается пояснение.
...
**BlindBox** — функция офлайн-очереди текста для закреплённого пира, когда **нет живой защищённой сессии**. Для **именованных** профилей (`persistent`) включена по умолчанию; для `default` (`transient`) отключена.
...
- Файлы истории создаются в папке профилей, формат имени:
...
- В меню **`⋯`** доступны переключатели **Notification sound**, **Hide message in notifications** и **Quiet mode (focused)** наряду с описанным ниже поведением.
