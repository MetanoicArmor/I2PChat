...
**Linux glibc baseline:** for release zips that run on common LTS distros, prefer building on **Ubuntu 22.04** (or use CI). Workflow **[`build-linux-release-artifacts.yml`](../.github/workflows/build-linux-release-artifacts.yml)** (`workflow_dispatch`) runs `build-linux.sh` on **ubuntu-22.04** and uploads the two zips + `SHA256SUMS` to the release (`--clobber`). Inputs: **`tag`** (release to upload to), optional **`source_ref`** (ref to build from; default = same as tag). If the checked-out ref has **no `I2PChat-tui.spec`**, the workflow **shallow-fetches `origin/main`** and continues there when **`VERSION` on `main` matches** the release (same tradeoff as manually setting **`source_ref: main`**). Otherwise set **`source_ref`** explicitly. Local builds on bleeding-edge distros can embed a **newer minimum glibc** than users on Debian/Ubuntu LTS have.

**Optional Docker (glibc 2.39):** to reproduce a **Ubuntu 24.04**-style toolchain locally, use **[`packaging/docker/README.md`](../packaging/docker/README.md)** — `./packaging/docker/run-linux-build.sh` builds an image from **`Dockerfile.linux-noble-glibc239`** and runs `./build-linux.sh` inside it. That baseline is **newer** than 22.04; do **not** replace the default release CI with this image unless you intentionally raise the minimum glibc for users.
