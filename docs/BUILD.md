...
**Linux glibc baseline:** for release zips that run on common LTS distros, prefer building on **Ubuntu 22.04** (or use CI). Workflow **[`build-linux-release-artifacts.yml`](../.github/workflows/build-linux-release-artifacts.yml)** (`workflow_dispatch`) runs `build-linux.sh` on **ubuntu-22.04** and uploads the two zips + `SHA256SUMS` to the release (`--clobber`). Inputs: **`tag`** (release to upload to), optional **`source_ref`** (ref to build from; default = same as tag). Tags **older than the TUI zip / `I2PChat-tui.spec` era** do not produce `I2PChat-linux-*-tui-*.zip` when checked out — use **`source_ref: main`** only if **`VERSION` on `main` still equals that release** (you are trading strict tag reproducibility for current packaging scripts). Local builds on bleeding-edge distros can embed a **newer minimum glibc** than users on Debian/Ubuntu LTS have.
