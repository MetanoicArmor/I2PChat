...
- **выбор из списка**:
  - открыть список справа от поля и выбрать уже существующий профиль (созданный ранее `.dat`‑файл в `~/.i2pchat`);
- **ввод нового имени**:
  - напечатать своё имя профиля (например, `alice`);
  - при дальнейшем использовании функции `Lock to peer` ключи и информация будут сохранены в `~/.i2pchat/<имя>.dat`.
...
Кнопка **`Lock to peer`** позволяет «привязать» текущий профиль к конкретному пиру.  
Это значит, что:
...
4. В остальных случаях:
   - создаётся/обновляется файл `~/.i2pchat/<имя_профиля>.dat`, куда дописывается адрес пира;
   - в чате появляется системное сообщение:
...
1. Открывается диалог `Select profile (.dat)`:
   - по умолчанию указывает на директорию профилей `~/.i2pchat`;
   - фильтрует файлы по маске `*.dat`.
2. Если файл не выбран — операция отменяется.
3. Если файл выбран:
   - из пути берётся имя файла без расширения (`<base>`);
   - `.dat` копируется в директорию профилей `~/.i2pchat/<base>.dat` (если ещё не там);
...
### 5. Изображения в ASCII/braille‑виде

Внутри GUI есть поддержка отправки изображений в виде ASCII/braille‑арта, используя функции `render_braille` и `render_bw` из ядра.

В коде `ChatWindow` присутствуют обработчики:

- `on_send_img_braille_clicked` — отправка изображения в braille‑формате;
- `on_send_img_bw_clicked` — отправка изображения в чёрно‑белом ASCII‑формате.

Они работают по одному принципу:

1. Открывается диалог выбора изображения:
   - `Select image to send (braille)` или
   - `Select image to send (bw)`.
2. Изображение переводится в набор строк (`lines`) с помощью подходящей функции рендеринга.
3. Полученный текстовый арт:
   - отображается в чате как ваше сообщение (`kind="image_braille"` или `"image_bw"`);
   - отправляется собеседнику как набор строк через ядро (`send_image_lines(lines)`).

В текущей версии GUI эти обработчики не привязаны к отдельным кнопкам на панели действий,  
но их можно вызывать программно или добавить соответствующие кнопки/пункты меню при дальнейшем развитии интерфейса.

### 6. Системные уведомления и звук
...
- Если модуль `QtMultimedia` доступен:
  ...
  - на macOS используется один из стандартных системных звуков (`/System/Library/Sounds/Glass.aiff`);
  - громкость по умолчанию — около 70%.
- При входящем сообщении, когда окно не активно:
  - проигрывается мягкий системный звук;
  - если проигрывание не удалось — используется резервный `QApplication.beep()`.

### 7. Типичные сценарии использования

#### 7.1. Первый запуск и отправка сообщения

1. Убедитесь, что:
   - установлен Python 3.9;
   - запущен I2P‑роутер с SAM (`127.0.0.1:7656`);
...
#### 7.2. Отправка файла собеседнику
...
#### 7.3. Переключение на постоянный профиль и «локация» пира

1. Запустите GUI с именем профиля:
...
#### 7.4. Импорт готового профиля из `.dat`
...
4. В открывшемся диалоге выберите `friend.dat`:
   - файл будет скопирован в `~/.i2pchat/friend.dat` (если ещё не там);
   - профиль автоматически переключится на `friend`;
   - состояние ядра будет перезапущено под новым профилем.

### 8. Типичные проблемы на уровне GUI

#### 8.1. В чате не появляются сообщения
...
#### 8.2. Не удаётся подключиться к пиру
...
#### 8.3. Не видно запросов на приём файла
...
#### 8.4. Не работает копирование текста сообщений
...
1. Запустить `python main_qt.py` (с нужным профилем).
