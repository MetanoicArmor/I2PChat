...
Добавлен отдельный модуль `protocol_codec.py`:
...
- `protocol_codec.py` — новый vNext codec и framing.
- `i2p_chat_core.py` — интеграция codec, security hardening, ACK validation, telemetry.
...
- `main_qt.py` — краткий `ACKdrop` в статусе.
- `chat-python.py` — краткий `ACKdrop` в status panel.
