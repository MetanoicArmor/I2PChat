...
Готовые манифесты для [Windows Package Manager](https://learn.microsoft.com/windows/package-manager/) лежат в каталоге с версией, например [`1.2.2/`](1.2.2/). Старые версии (например 1.2.1) при необходимости ищите в истории git этого репозитория — в дереве остаётся только актуальный набор файлов, чтобы не путать его с шаблоном для PR.
